...
/** Build a SyntaxKind bitmap for simple ESTree trigger types. */
export function buildSyntaxKindBitmap(estreeTypes: Iterable<string>): {
	bitmap: Uint8Array;
	...
	return { bitmap: simpleBitmap, hasConditional };
}

export function predicateForTriggerSet(estreeTypes: Iterable<string>): Predicate {
	const { bitmap: simpleBitmap } = buildSyntaxKindBitmap(estreeTypes);
	let simpleCount = 0;
	for (let i = 0; i < SK_BITMAP_SIZE; i++) if (simpleBitmap[i]) simpleCount++;
