...
const lazy = require('../lib/lazy-estree.js');
...
function eagerConvert(sf) {
    return astConverter(sf, PARSE_SETTINGS, true).estree;
}
const failures = [];
...
// separately).
function compare(lazyNode, eagerNode, path) {
...
// shouldn't be mapped yet. After reading, it should be.
{
    ...
    check('lazy: child not in tsNodeToESTreeNodeMap before access', !astMaps.tsNodeToESTreeNodeMap.has(tsStatement));
    void estree.body[0];
    check('lazy: child in tsNodeToESTreeNodeMap after access', astMaps.tsNodeToESTreeNodeMap.has(tsStatement));
}
...
// The lookup must build the parent chain on the way and cache each step.
{
    ...
    const tsStmt2 = sf.statements[1];
    const tsDecl2 = tsStmt2.declarationList.declarations[0];
    const tsId2 = tsDecl2.name;
    // Top-down hasn't touched body yet — caches are empty for body[*].
    check('bottom-up: parent statement not cached before lookup', !context.maps.tsNodeToESTreeNodeMap.has(tsStmt2));
    // Materialise the deepest target.
    const idEstree = lazy.materialize(tsId2, context);
    check('bottom-up: returns ESTree Identifier', idEstree.type === 'Identifier');
    check('bottom-up: name matches', idEstree.name === 'y');
    // Walking up via `parent` should land on Program.
    const declarator = idEstree.parent;
...
// asking for child_b should reuse the existing parent (not create a new one).
{
    ...
    const parentInstance = declAEstree.parent; // VariableDeclaration
    // Now build child_b. It should find parentInstance via cache and reuse.
    const declBEstree = lazy.materialize(tsDeclB, context);
    check('sibling reuse: child_b.parent === parentInstance (no fresh creation)', declBEstree.parent === parentInstance);
    void estree;
}
...
// call is just a cache hit, no fresh construction.
{
    ...
    const tsId = sf.statements[0].declarationList.declarations[0].name;
...
// getter, materialise() should return that exact instance.
{
    ...
    const topDownId = (estree.body[0]).declarations[0].id;
    // Now bottom-up:
...
// must skip past it.
{
    const sf = parseTs('let x = 1;');
    const { estree, context } = lazy.convertLazy(sf);
    const tsDecl = sf.statements[0].declarationList.declarations[0];
    // tsDecl.parent.kind === VariableDeclarationList
    check('TS-only ancestor exists', tsDecl.parent.kind === ts.SyntaxKind.VariableDeclarationList);
    ...
    const declStmt = declarator.parent;
...
// every intermediate level once.
{
    ...
    const { estree, context } = lazy.convertLazy(sf);
    const tsId = (sf.statements[1].declarationList.declarations[0].name);
...
// the wrapper-creating getter on the parent instead.
{
    ...
    const idTopDown = (estree.body[0]).declarations[0].id;
    const wrapperTopDown = idTopDown.typeAnnotation;
    const innerTopDown = wrapperTopDown.typeAnnotation;
    check('top-down inner type parent === TSTypeAnnotation wrapper', innerTopDown.parent?.type === 'TSTypeAnnotation');
...
// reference and survive WeakSet membership checks.
{
    ...
    const declarator2 = estree.body[0].declarations[0];
...
// Set<LazyNode> with mixed access patterns — top-down then bottom-up.
{
    ...
    const { estree, context } = lazy.convertLazy(sf);
    const tsId = sf.statements[0].declarationList.declarations[0].name;
    const topDown = (estree.body[0]).declarations[0].id;
    const seen = new Set([topDown]);
...
// (convertChild's null path), not undefined. Match it.
{
    ...
    const { estree } = lazy.convertLazy(sf);
    const declarator = (estree.body[0]).declarations[0];
...
// (literal undefined, not null). Confirm parity.
{
    const sf = parseTs('let x = 1;');
    const { estree } = lazy.convertLazy(sf);
    const id = (estree.body[0]).declarations[0].id;
...
// Equivalent nodes from the two trees are NOT === to each other.
{
    ...
    const idA = (a.estree.body[0]).declarations[0].id;
    const idB = (b.estree.body[0]).declarations[0].id;
...
// .find, .map all work. Materialised eagerly on first read but cached.
{
    ...
    const mapped = body.map((n) => n.type);
    check('iteration: .map preserves length', mapped.length === 3);
    check('iteration: .find returns identity', body.find((n) => n.type === 'VariableDeclaration') === body[0]);
}
...
// - esTreeNodeToTSNodeMap[wrapper] is undefined (wrapper is not registered).
{
    ...
    const tsType = sf.statements[0].declarationList.declarations[0].type;
    check('wrapper: tsNodeToESTreeNodeMap[tsType] points at inner (not wrapper)', astMaps.tsNodeToESTreeNodeMap.get(tsType) === inner);
    check('wrapper: esTreeNodeToTSNodeMap[wrapper] is undefined (not registered)', astMaps.esTreeNodeToTSNodeMap.get(wrapper) === undefined);
    check('wrapper: esTreeNodeToTSNodeMap[inner] is the TS type node', astMaps.esTreeNodeToTSNodeMap.get(inner) === tsType);
...
// directly so a regression is loud.
{
    ...
    const lazyAs = (estree.body[0]).declarations[0].init;
...
// CRLF newlines: line counters must increment on \r\n once, not twice.
{
    ...
    const { estree } = lazy.convertLazy(sf);
    const eager = eagerConvert(parseTs(code));
    const lazyB = (estree.body[1]).declarations[0].id;
...
// must still match eager.
{
    ...
    const { estree } = lazy.convertLazy(sf);
    const eager = eagerConvert(parseTs(code));
    const lazyId = (estree.body[0]).declarations[0].id;
    const eagerId = eager.body[0].declarations[0].id;
    check('no-trailing-nl: id range matches eager', lazyId.range[0] === eagerId.range[0] && lazyId.range[1] === eagerId.range[1]);
...
// Empty source: Program with empty body, range [0, 0].
{
    ...
    check('empty source: lazy body length 0', estree.body.length === 0);
    check('empty source: range [0,0]', estree.range[0] === eager.range[0] && estree.range[1] === eager.range[1]);
    check('empty source: loc shape matches', estree.loc.start.line === eager.loc.start.line
...
// 0 because SourceFile.getStart() === SourceFile.pos === 0. Verify parity.
{
    ...
    const { estree } = lazy.convertLazy(sf);
    const eager = eagerConvert(parseTs(code));
    check('whitespace-only: range matches eager', estree.range[0] === eager.range[0] && estree.range[1] === eager.range[1]);
}
...
// wrapper-creating getter on the parent.
{
    ...
    check('wrapper bottom-up: inner type is TSNumberKeyword', innerEstree.type === 'TSNumberKeyword');
    check('wrapper bottom-up: inner.parent is TSTypeAnnotation wrapper', innerEstree.parent?.type === 'TSTypeAnnotation');
    check('wrapper bottom-up: wrapper.parent is Identifier', innerEstree.parent?.parent?.type === 'Identifier');
}
...
// for any kind sitting in a `type` slot.
{
    ...
    const tsType = sf.statements[0].declarationList.declarations[0].type;
    const innerEstree = lazy.materialize(tsType, context);
    check('wrapper bottom-up: TSTypeReference inner type', innerEstree.type === 'TSTypeReference');
    check('wrapper bottom-up: TSTypeReference parent is wrapper', innerEstree.parent?.type === 'TSTypeAnnotation');
}
...
// getter cleanly (cache hit, not a fresh build with a different parent).
{
    ...
    const { estree, context } = lazy.convertLazy(sf);
    const tsType = sf.statements[0].declarationList.declarations[0].type;
    const topDown = (estree.body[0]).declarations[0].id.typeAnnotation.typeAnnotation;
...
// dictates the next batch of LazyNode classes to add.
{
    ...
    const walkDir = (dir) => {
        for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
            ...
            if (entry.isDirectory()) {
                if (entry.name === 'node_modules' || entry.name === 'upstream' || entry.name === 'test')
                    continue;
                walkDir(full);
            }
    ...
    for (const rel of targets) {
        ...
        try {
            ...
            walked.push({ file: rel, msg: err.message.split('\n')[0] });
        }
    }
    check(`real-source walk: ${passed}/${targets.length} TS files under packages/`, walked.length === 0, walked.length ? `first failure: ${walked[0].file} → ${walked[0].msg}` : '');
    ...
    check(`real-source parity: ${targets.length - parityResults.length}/${targets.length} files match eager byte-for-byte`, parityResults.length === 0, parityResults.length ? `${parityResults.length} files diverge` : '');
...
// arrays, parent chains) is iteratable end-to-end.
{
    ...
    walk(estree);
    check(`smoke: visits TSAnyKeyword 3 times (one per 'any' in source) — got ${visits.TSAnyKeyword ?? 0}`, visits.TSAnyKeyword === 3);
    check(`smoke: visits Identifier (canonical TS-aware visit) — got ${visits.Identifier ?? 0}`, (visits.Identifier ?? 0) >= 3);
    check(`smoke: visits TSTypeAnnotation wrapper — got ${visits.TSTypeAnnotation ?? 0}`, (visits.TSTypeAnnotation ?? 0) === 3);
    check(`smoke: visits FunctionDeclaration — got ${visits.FunctionDeclaration ?? 0}`, visits.FunctionDeclaration === 1);
    check(`smoke: visits VariableDeclaration — got ${visits.VariableDeclaration ?? 0}`, visits.VariableDeclaration === 1);
}
// --- Future / blocked --------------------------------------------------
//
...
//# sourceMappingURL=lazy-estree.test.js.map