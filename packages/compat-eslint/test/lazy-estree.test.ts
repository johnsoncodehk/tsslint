...
// exercise. All must match eager byte-for-byte; regression alarm if they
// stop matching.
{
	const supportedFixtures: Array<{ name: string; code: string }> = [
		...
		{ name: 'object-destructure-with-default', code: `let { a = 1, b: c = 2 } = { a: 1 };` },
		...
		{ name: 'enum-string', code: `enum E { A = 'a', B = 'b' }` },
		{ name: 'enum-computed', code: `enum E { A = 1 << 0, B = 1 << 1 }` },
		{ name: 'enum-export', code: `export enum E { A }` },
		{ name: 'const-enum', code: `const enum E { A, B }` },
		...
		{ name: 'mapped-type-as', code: `type Renamed<T> = { [K in keyof T as \`get_\${string & K}\`]: T[K] };` },
		// Modules / namespaces
		...
		{ name: 'tagged-template', code: 'function tag(s: TemplateStringsArray, ...v: any[]) {} tag`hello ${1} world`;' },
		{ name: 'template-literal-type', code: 'type Greeting<T extends string> = `hello ${T}`;' },
		// new.target / import.meta
		{ name: 'new-target', code: `function F() { return new.target; }` },
		{ name: 'import-meta', code: `let url = import.meta.url;` },
		// import equals (CJS interop)
		{ name: 'import-equals', code: `import x = require('x');` },
	];

	const runFixture = (fx: { name: string; code: string }): { matched: boolean; first?: string } => {
		...
		try {
			...
			const eagerEstree = eagerConvert(eagerSf);
			const beforeFails = failures.length;
			compare(lazyEstree, eagerEstree, `supported:${fx.name}`);
			const newFails = failures.slice(beforeFails);
	...
	const supportedDiverged: Array<{ name: string; first: string }> = [];
...
// any TS-only kind that typescript-estree elides.
{
	...
	for (const rel of DOGFOOD_FILES) {
		...
		// Splice off the failures the helper just appended for this file
		// so we can report dogfood ones separately from the curated
		// fixture sweep above.
		const newFailures = fixtureFailures.splice(beforeF, fixtureFailures.length - beforeF);
		dogfoodFailures.push(...newFailures);
	}
	check(
		`bottom-up parity sweep (dogfood): ${DOGFOOD_FILES.length} files, ${dogfoodNodes} type asserts, ${dogfoodParents} parent asserts`,
		dogfoodFailures.length === 0,
		dogfoodFailures.length
			? '\n      ' + dogfoodFailures.map(([n, m]) => `[${n}] ${m}`).join('\n      ')
			: undefined,
	);
}
