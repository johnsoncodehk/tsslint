...
defineShape<ts.ImportSpecifier>(SK.ImportSpecifier, {
	...
	slots: {
		...
		imported: { compute: (tn: ts.ImportSpecifier, parent) =>
			convertChild(tn.propertyName ?? tn.name, parent) },
	},
});
defineShape<ts.ExportSpecifier>(SK.ExportSpecifier, {
	...
	slots: {
		...
		local: { compute: (tn: ts.ExportSpecifier, parent) =>
			convertChild(tn.propertyName ?? tn.name, parent) },
	},
});
...
defineShape<ts.InterfaceDeclaration>(SK.InterfaceDeclaration, {
	...
	slots: {
		...
		extends: { compute: (tn: ts.InterfaceDeclaration, parent) => {
		...
		typeParameters: { tsField: 'typeParameters', via: convertTypeParameters, whenAbsent: 'undefined' },
		body: { compute: (tn: ts.InterfaceDeclaration, parent) =>
			new TSInterfaceBodyNode(tn, parent) },
	},
});
...
const tsRestNamedTupleShape = makeShapeClass<ts.NamedTupleMember>({
	...
	slots: {
		...
		typeAnnotation: { compute: (tn: ts.NamedTupleMember, parent) => {
...
const memberExpressionShape = makeShapeClass<ts.PropertyAccessExpression | ts.ElementAccessExpression>({
	...
	slots: {
		...
		property: { compute: (tn: ts.PropertyAccessExpression | ts.ElementAccessExpression, parent) =>
...
const sequenceExpressionShape = makeShapeClass<ts.BinaryExpression>({
	...
	slots: {
		expressions: { compute: (be: ts.BinaryExpression, parent) => {
...
const jsxExpressionContainerShape = makeShapeClass<ts.JsxExpression>({
	...
	slots: {
		...
		expression: { compute: (tn: ts.JsxExpression, parent) =>
			tn.expression
...
const classShape = makeShapeClass<ts.ClassDeclaration | ts.ClassExpression>({
	...
	slots: {
		id: { tsField: 'name' },
		typeParameters: { tsField: 'typeParameters', via: convertTypeParameters, whenAbsent: 'undefined' },
		superClass: { compute: (tn: ts.ClassDeclaration | ts.ClassExpression, parent) => {
			const ext = tn.heritageClauses?.find(c => c.token === SK.ExtendsKeyword);
			return ext ? convertChild(ext.types[0]?.expression, parent) : null;
		} },
		superTypeArguments: { compute: (tn: ts.ClassDeclaration | ts.ClassExpression, parent) => {
			const ext = tn.heritageClauses?.find(c => c.token === SK.ExtendsKeyword);
			const args = ext?.types[0]?.typeArguments;
			return args ? convertTypeArguments(args, parent) : undefined;
		} },
		implements: { compute: (tn: ts.ClassDeclaration | ts.ClassExpression, parent) => {
		...
		body: { compute: (tn: ts.ClassDeclaration | ts.ClassExpression, parent) =>
			new ClassBodyNode(tn, parent) },
	},
});
...
const objectBindingElementShape = makeShapeClass<ts.BindingElement>({
	...
	slots: {
		key: { compute: (tn: ts.BindingElement, parent) =>
			convertChild(tn.propertyName ?? tn.name, parent) },
		value: { compute: (tn: ts.BindingElement, parent) => {
...
const objectMethodShape = makeShapeClass<ts.MethodDeclaration>({
	...
	slots: {
		key: { tsField: 'name' },
		value: { compute: (tn: ts.MethodDeclaration, parent) =>
...
const objectAccessorShape = (kind: 'get' | 'set') =>
	makeShapeClass<ts.GetAccessorDeclaration | ts.SetAccessorDeclaration>({
		...
		slots: {
			key: { tsField: 'name' },
			value: { compute: (tn: ts.GetAccessorDeclaration | ts.SetAccessorDeclaration, parent) =>
				new MethodFunctionExpressionNode(tn, parent) },
		},
	});
...
const methodDefinitionShape = makeShapeClass<ts.MethodDeclaration | ts.ConstructorDeclaration | ts.GetAccessorDeclaration | ts.SetAccessorDeclaration>({
	...
	slots: {
		...
		key: { compute: (tn: ts.MethodDeclaration | ts.ConstructorDeclaration, parent) => {
			if (tn.kind === SK.Constructor) {
				return new ConstructorKeyIdentifierNode(tn, parent);
			}
			return convertChild((tn as ts.MethodDeclaration).name, parent);
		} },
		value: { compute: (tn: ts.MethodDeclaration | ts.ConstructorDeclaration, parent) =>
			new MethodFunctionExpressionNode(tn, parent) },
		decorators: { compute: (tn, parent) => convertDecorators(tn, parent) },
	},
});
...
const jsxElementShape = makeShapeClass<ts.JsxElement | ts.JsxSelfClosingElement>({
	type: 'JSXElement',
	slots: {
		openingElement: { compute: (tn: ts.JsxElement | ts.JsxSelfClosingElement, parent) => {
		...
		closingElement: { compute: (tn: ts.JsxElement | ts.JsxSelfClosingElement, parent) =>
			tn.kind === SK.JsxSelfClosingElement
				? null
				: convertChild(tn.closingElement, parent) },
		children: { compute: (tn: ts.JsxElement | ts.JsxSelfClosingElement, parent) =>
			tn.kind === SK.JsxSelfClosingElement
				? EMPTY_ARRAY
...
defineShape<ts.ImportDeclaration>(SK.ImportDeclaration, {
	...
	slots: {
		source: { tsField: 'moduleSpecifier' },
		specifiers: { compute: (tn: ts.ImportDeclaration, parent) => {
			const out: (LazyNode | null)[] = [];
...
function wrapChainIfNeeded(
	...
	const r = result as unknown as { type: string; object?: LazyNode; callee?: LazyNode; optional?: boolean; _ctx: ConvertContext };
...
const ClassBodyNode = makeShapeClass<ts.ClassDeclaration | ts.ClassExpression>({
	...
	slots: {
		body: { tsField: 'members', via: (members, parent) =>
			convertChildren((members as ts.NodeArray<ts.ClassElement>).filter(m => m.kind !== SK.SemicolonClassElement), parent) },
	},
});
...
const MethodFunctionExpressionNode = makeShapeClass<ts.MethodDeclaration | ts.ConstructorDeclaration | ts.GetAccessorDeclaration | ts.SetAccessorDeclaration>({
...
const jsxOpeningElementShape = makeShapeClass<ts.JsxOpeningElement | ts.JsxSelfClosingElement>({
	...
	slots: {
		...
		attributes: { tsField: 'attributes', via: (attrs, parent) =>
			convertChildren((attrs as ts.JsxAttributes).properties, parent) },
		typeArguments: { tsField: 'typeArguments', via: convertTypeArguments, whenAbsent: 'undefined' },
	},
});
defineShapeRouter(SK.JsxOpeningElement, (tsNode, parent) =>
	new jsxOpeningElementShape(tsNode as ts.JsxOpeningElement, parent));
...
const jsxIdentifierShape = makeShapeClass<ts.Identifier | ts.ThisExpression>({
	...
	consts: tn => ({
		...
		name: tn.kind === SK.ThisKeyword ? 'this' : (tn as unknown as ts.Identifier).text,
	}),
	slots: {},
});
const jsxMemberExpressionShape = makeShapeClass<ts.PropertyAccessExpression>({
	...
	slots: {
		...
		property: { tsField: 'name', via: (name, parent) =>
			new jsxIdentifierShape(name as ts.Identifier, parent) },
	},
});
const jsxNamespacedNameShape = makeShapeClass<ts.JsxNamespacedName>({
	type: 'JSXNamespacedName',
	slots: {
		namespace: { tsField: 'namespace', via: (ns, parent) =>
			new jsxIdentifierShape(ns as ts.Identifier, parent) },
		name: { tsField: 'name', via: (nm, parent) =>
			new jsxIdentifierShape(nm as ts.Identifier, parent) },
	},
});
...
defineShapeRouter(SK.JsxNamespacedName, (tsNode, parent) =>
	new jsxNamespacedNameShape(tsNode as ts.JsxNamespacedName, parent));
...
function convertJSXTagName(node: ts.Node, parent: LazyNode): LazyNode {
	if (node.kind === SK.PropertyAccessExpression) {
		return new jsxMemberExpressionShape(node as ts.PropertyAccessExpression, parent);
	}
	if (node.kind === SK.JsxNamespacedName) {
		return new jsxNamespacedNameShape(node as ts.JsxNamespacedName, parent);
	}
	return new jsxIdentifierShape(node as ts.Identifier, parent);
}
